...
fn main() {
    ...
    let collection = DeviceInformation::find_all_async_aqs_filter(&ref_selector).unwrap().blocking_get().expect("find_all_sync failed").unwrap();
    let mut count = 1;
    ...
    for device_info in collection.into_iter() {
        ...
        let service = GattDeviceService::from_id_async(&ref_id).unwrap().blocking_get().expect("from_id_async failed").unwrap();
        let sound = service.get_characteristics(toio_sound_uuid).unwrap().unwrap();
        for sd in sound.into_iter() {
            ...
            sd.write_value_async(&buffer).unwrap().blocking_get().expect("failed");
        }
            
        let motor = service.get_characteristics(toio_motor_uuid).unwrap().unwrap();
        for mt in motor.into_iter() {
            let mt = mt.unwrap();
            println!("{}", mt.get_user_description().unwrap());
            let writer = DataWriter::new();
            writer.write_bytes(&[0x02, 0x01, 0x01, 0x10, 0x02, 0x01, 0x10, 0x40]).expect("error");
            let buffer = writer.detach_buffer().expect("error").unwrap();
            mt.write_value_async(&buffer).unwrap().blocking_get().expect("failed");
        }
    } 
    println!("Hello, world!");
}
